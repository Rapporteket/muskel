...
ForlopsData <- read.table('P:/MinData/muskel/ForlopsOversikt2017-09-13 15-40-10.txt', header=TRUE, sep=';')
...
RegData <- read.table('P:/MinData/muskel/AlleVarNum2017-09-13 15-40-08.txt', header=TRUE, sep=';')
...
RegDataLabel <- read.table('P:/MinData/muskel/AlleVar2017-09-13 15-40-05.txt', header=TRUE, sep=';')
...
## Oppfølginger
BasisReg <- RegData[difftime(as.POSIXlt('2016-12-31'), RegData$HovedDato) > 5*365.25 & RegData$Avdod == 'Nei' &
                     RegData$ForlopsType1Num==1, ]
Oppfolging <- RegData[RegData$ForlopsType1Num==2 & RegData$PasientID %in% BasisReg$PasientID, ]


OppfolgPst2014 <- length(intersect(BasisReg$PasientID[BasisReg$Aar<=2009], Oppfolging$PasientID[Oppfolging$Aar==2014]))/dim(BasisReg[BasisReg$Aar<=2009,])[1]*100
OppfolgPst2015 <- length(intersect(BasisReg$PasientID[BasisReg$Aar<=2010], Oppfolging$PasientID[Oppfolging$Aar<=2015]))/dim(BasisReg[BasisReg$Aar<=2010,])[1]*100
OppfolgPst2016 <- length(intersect(BasisReg$PasientID[BasisReg$Aar<=2011], Oppfolging$PasientID[Oppfolging$Aar<=2016]))/dim(BasisReg[BasisReg$Aar<=2011,])[1]*100

OppfolgPst <- data.frame('2014'=OppfolgPst2014, '2015'=OppfolgPst2015, '2016'=OppfolgPst2016)
names(OppfolgPst) <- c('2014', '2015', '2016')
...
MuskelFigAndeler(RegData=RegData, valgtVar='SMA', datoFra=datoFra, datoTil=datoTil, reshID=reshID, diagnosegr=diagnosegr,
                       minald=minald, maxald=17, erMann=erMann, outfile=outfile, diagnoseSatt=diagnoseSatt, forlop=forlop,
                       enhetsUtvalg=enhetsUtvalg, preprosess=preprosess, hentData=hentData)
outfile='SMAfom18.pdf'
MuskelFigAndeler(RegData=RegData, valgtVar='SMA', datoFra=datoFra, datoTil=datoTil, reshID=reshID, diagnosegr=diagnosegr,
                       minald=18, maxald=maxald, erMann=erMann, outfile=outfile, diagnoseSatt=diagnoseSatt, forlop=forlop,
                       enhetsUtvalg=enhetsUtvalg, preprosess=preprosess, hentData=hentData)
