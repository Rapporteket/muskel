...
\title{Figurer og tabeller til årsrapport for Muskelregisteret 2020}
...
Rapportaar <- 2020
datoFra='2008-01-01'
datoTil=paste0(Rapportaar, "-12-31")
...
library(xtable)
## Oppfølginger
BasisReg <- RegData[difftime(as.POSIXlt(datoTil), RegData$HovedDato) > 5*365.25 & RegData$Avdod == 'Nei' &
...
## Andel genetisk bekreftet diagnose i perioden Rapportaar-4-Rapportaar etter de fire helseregionene
## Versjon 1, tom og ukjent fjernes

subdata <- RegData[RegData$Aar %in% (Rapportaar-4):2020, ]
...
MuskelFigAndelGrvar(RegData=subdata, valgtVar= "GenetiskAarsakPaavist", datoFra=paste0(Rapportaar-4, "-01-01"), datoTil=datoTil,
                               diagnosegr=-1, grvar="rhf", minald=0, maxald=120, erMann=99, outfile=outfile, forlop = 99,
                               diagnose=-1, undergr=-1, skriftstr = 1.0, undergr2=-1, avdod='Nei', inkl_tittel=T, decreasing = T,
                               gen_aarsak_paavist=-1, xtekst='Andel', graa = '', tittel='Andel genetisk verifisert diagnose')


subdata <- RegData[RegData$Aar %in% (Rapportaar-4):Rapportaar, ]
subdata <- subdata[order(subdata$HovedDato, decreasing = T), ]
subdata <- subdata[match(unique(subdata$PasientID), subdata$PasientID), ]
...
tabell <- MuskelFigAndelGrvar(RegData=subdata, valgtVar= "GenetiskAarsakPaavist", datoFra=paste0(Rapportaar-4, "-01-01"), datoTil=datoTil,
                               diagnosegr=1, grvar="rhf", minald=0, maxald=120, erMann=99, outfile=outfile, forlop = 99,
                               diagnose=-1, undergr=-1, skriftstr = 1.0, undergr2=-1, avdod='Nei', inkl_tittel=T, decreasing = T,
                               gen_aarsak_paavist=-1, xtekst='Andel', graa = '', tittel='Andel genetisk verifisert diagnose')
tabell <- tabell[-6, c(5,2,3,4)]; names(tabell)[1] <- "RHF"
write.csv2(tabell, "I:/muskel/andel_gen_ver_pr_borhf_muskel.csv", row.names = F, fileEncoding = "Latin1")
...
tabell <- MuskelFigAndelGrvar(RegData=subdata, valgtVar= "GenetiskAarsakPaavist", datoFra=paste0(Rapportaar-4, "-01-01"), datoTil=datoTil,
                               ...
                               gen_aarsak_paavist=-1, xtekst='Andel', graa = '', tittel='Andel genetisk verifisert diagnose')
tabell <- tabell[-6, c(5,2,3,4)]; names(tabell)[1] <- "RHF"
write.csv2(tabell, "I:/muskel/andel_gen_ver_pr_borhf_sma.csv", row.names = F, fileEncoding = "Latin1")

outfile <- "andel_gen_ver_pr_borhf_ploynevropati.pdf"
tabell <- MuskelFigAndelGrvar(RegData=subdata, valgtVar= "GenetiskAarsakPaavist", datoFra=paste0(Rapportaar-4, "-01-01"), datoTil=datoTil,
                               diagnosegr = 3, grvar="rhf", minald=0, maxald=120, erMann=99, outfile=outfile, forlop = 99,
                               undergr=-1, skriftstr = 1.0, undergr2=-1, avdod='Nei', inkl_tittel=T, decreasing = T,
                               gen_aarsak_paavist=-1, xtekst='Andel', graa = '', tittel='Andel genetisk verifisert diagnose')
tabell <- tabell[-6, c(5,2,3,4)]; names(tabell)[1] <- "RHF"
write.csv2(tabell, "I:/muskel/andel_gen_ver_pr_borhf_ploynevropati.csv", row.names = F, fileEncoding = "Latin1")



# diagnosegr=1, diagnose = 'G71.9''G71.9'

## tid fra utredning til diagnose

subdata <- RegData[which(RegData$Utredningsstart >= (Rapportaar-4) & RegData$Utredningsstart <= Rapportaar), ]
...
MuskelFigGjsnGrvar(RegData=subdata, valgtVar= "TidUtredDiag", datoFra=paste0(Rapportaar-4, "-01-01"), datoTil=datoTil,
                               diagnosegr=-1, grvar="rhf", minald=0, maxald=120, erMann=99, outfile=outfile, forlop = 99,
                               diagnose=-1, undergr=-1, skriftstr = 1.0, undergr2=-1, avdod='', inkl_tittel=T, decreasing = T,
                               gen_aarsak_paavist=-1, xtekst='Gjennomsnitt', graa = '', tittel=c('Gj.snittlig tid fra utredning til diagnose', 'av de med diagnoseår registrert'), UtredningsaarFra=Rapportaar-4, UtredningsaarTil=Rapportaar)
...
\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{andel_gen_ver_pr_borhf_v1.pdf}
\caption{andel\_gen\_ver\_pr\_borhf\_v1.pdf}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{andel_gen_ver_pr_borhf_v2.pdf}
\caption{andel\_gen\_ver\_pr\_borhf\_v2.pdf}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{andel_gen_ver_pr_borhf_muskel.pdf}
\caption{andel\_gen\_ver\_pr\_borhf\_muskel.pdf}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{andel_gen_ver_pr_borhf_sma.pdf}
\caption{andel\_gen\_ver\_pr\_borhf\_sma.pdf}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{andel_gen_ver_pr_borhf_ploynevropati.pdf}
\caption{andel\_gen\_ver\_pr\_borhf\_ploynevropati.pdf}
\end{figure}
